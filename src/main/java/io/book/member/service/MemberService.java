...
import io.book.member.domain.PasswordEncoder;
...
public class MemberService {
    ...
    public MemberDto join(final MemberParameter parameter) {
        ...
        Member member = repository.save(Member.of(parameter.getKey(), PasswordEncoder.encode(parameter.getPassword())));
        return toDto(member);
    }

    public void login(final MemberParameter parameter) {
        Member member = load(parameter.getKey());
        if (!member.isValidPassword(PasswordEncoder.ENCODER, parameter.getPassword())) {
